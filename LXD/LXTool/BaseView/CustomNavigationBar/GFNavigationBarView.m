...
//导航条颜色设置
#define ColorNaviBar RGB(255,0,0)
#define ColorNaviBar_ItemText [UIColor whiteColor]
#define FontNaviBar_ItemText [UIFont boldSystemFontOfSize:18]
...
- (void)createNaviBarView{
    ...
    _titleLabel.text = @"标题";
    _titleLabel.font = FontNaviBar_ItemText;//[UIFont fontWithName:@"PingFangSC-Medium" size:17];
    _titleLabel.textColor = ColorNaviBar_ItemText;
    ...
    _segmentationLine.hidden = YES;
    
    //********建立约束************
    //左边第一按钮
    [_leftFirstBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.naviBarView).offset(5);
        make.bottom.equalTo(self.naviBarView).offset(0);
    ...
    [_rightFirstBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.naviBarView.mas_right).with.offset(-5.);
        make.width.and.height.mas_equalTo(44);
    }];
    ...
    [_rightSecondBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.leftFirstBtn);
        make.right.equalTo(self.rightFirstBtn.mas_left).with.offset(-10.);
        make.width.and.height.mas_equalTo(44);
    }];
